...
// ゲームの情報
EsaModel g_aEsaModelResult[MAX_ESATYPE];					// エサのモデル
int g_nMaxPlayer;											// プレイヤーの総数
int g_nMaxEsaType;											// エサの種類の総数
WORD g_aEsaScore[MAX_ESATYPE];								// エサのスコア
WORD g_aNumHaveEsa[MAX_PLAYER][MAX_ESATYPE];				// 持っているエサの数
...
{// {インデックス, aブレンドをするか, 幅, 高さ, テクスチャ座標, テクスチャサイズ, 色}

	{0, false, SCREEN_WIDTH * 0.5f, SCREEN_HEIGHT * 0.5f, D3DXVECTOR2(0.0f,0.0f), D3DXVECTOR2(1.0f,1.0f), D3DXCOLOR(1.0f,1.0f,1.0f,1.0f)},	// [0]背景情報
};

Result_info g_aResultInfo[] =
{// {ポリゴンか, セットインデックス, 描画順位, 位置, 角度}

	{true, 0, 0, D3DXVECTOR3(SCREEN_WIDTH * 0.5f, SCREEN_HEIGHT * 0.5f, 0.0f), D3DXVECTOR3(0.0f,0.0f,0.0f)},
};
...
void InitResult(void)
{
	...
	// カメラの位置設定
	SetCameraPos(0, D3DXVECTOR3(0.0f, 100.0f, 100.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f), CAMERATYPE_POINT);

	// テクスチャの読み込み
	for (nCntResult = 0; nCntResult < sizeof c_apFilenameResult / sizeof(c_apFilenameResult[0]); nCntResult++)
	{
		hr = D3DXCreateTextureFromFile(pDevice,								// デバイス
								  c_apFilenameResult[nCntResult],		// テクスチャファイル名
								  &g_apTextureResult[nCntResult]);		// テクスチャポインタ
	}

	// Xファイルの読み込み
	if (FAILED(D3DXLoadMeshFromX("data/MODEL/esa/shrimp.x",
		D3DXMESH_SYSTEMMEM,
		pDevice,
		NULL,
		&g_aEsaModelResult[0].pBuffMat,
		NULL,
		&g_aEsaModelResult[0].dwNumMat,
		&g_aEsaModelResult[0].pMesh)))
	{// Xファイルの読み込みに失敗した場合

		return;	// 設定した場所がない事を返す
	}

	D3DXMATERIAL* pMat;	// マテリアルのポインタを宣言

	// マテリアルデータへのポインタを所得
	pMat = (D3DXMATERIAL*)g_aEsaModelResult[0].pBuffMat->GetBufferPointer();

	// マテリアルの設定
	for (int nCntMat = 0; nCntMat < (int)g_aEsaModelResult[0].dwNumMat; nCntMat++)
	{// マテリアルの数分繰り返す
		...
		{// テクスチャファイルがある
			...
			hr = D3DXCreateTextureFromFile(pDevice,								// Direct3Dデバイスへのポインタ
				...
				&g_aEsaModelResult[0].apTexture[nCntMat]);	// テクスチャへのポインタ
		}
	}
...
void UninitResult(void)
{
	...
	// ゲームの各情報を初期化
	for (nCntResult = 0; nCntResult < MAX_PLAYER; nCntResult++)
	{// プレイヤーの最大数分繰り返す
...
void UpdateResult(void)
{
	...
	{
		g_aResult[0].bPolygon = (g_aResult[0].bPolygon == true) ? false : true;

		if (g_aResult[0].bPolygon == true)
		{
			g_aResult[0].pos = D3DXVECTOR3(SCREEN_WIDTH * 0.5f, SCREEN_HEIGHT * 0.5f, 0.0f);
		}
		else
		{
	...
	{
		if (g_aResult[nCntResult].bUse == true)
		{// 使用している場合
			...
			pVtx += (nCntResult * 4);	// 頂点バッファをインデックス分進める
...
void DrawResultModel(Result* pResult)
{
	...
	// マテリアルデータへのポインタを所得
	pMat = (D3DXMATERIAL*)g_aEsaModelResult[pResult->nIdx].pBuffMat->GetBufferPointer();

	for (int nCntMat = 0; nCntMat < (int)g_aEsaModelResult[pResult->nIdx].dwNumMat; nCntMat++)
	{// マテリアルの数分繰り返す
		...
		// テクスチャの設定
		pDevice->SetTexture(0, g_aEsaModelResult[pResult->nIdx].apTexture[nCntMat]);

		// モデルパーツの描画
		g_aEsaModelResult[pResult->nIdx].pMesh->DrawSubset(nCntMat);
	}
	...
	for (int nCntResult = 0; nCntResult < MAX_NUM_RESULT; nCntResult++)
	{
		...
		{// 使用していない場合
			...
			{// モデルの場合
				...
				if (g_aEsaModelResult[nIdx].pMesh != NULL)
				{
