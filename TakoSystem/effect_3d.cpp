...
#define MAX_SET_EFFECT3D		(1280)							// エフェクトの最大数

#define EFFECT3D_FILENAME		"data\\TEXTURE\\effect000.jpg"	// 使用するエフェクトのファイル名
...
void UpdateEffect3D(void)
{
	...
	for (nCntEffect = 0; nCntEffect < MAX_SET_EFFECT3D; nCntEffect++)
	{
		...
		{// 使用されている場合
			...
			// 位置の更新
			g_aEffect3D[nCntEffect].pos += g_aEffect3D[nCntEffect].vecMove;
